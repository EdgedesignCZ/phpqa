...
```bash
docker run --rm -it ghcr.io/edgedesigncz/phpqa:v1.26.0-php7.2 phpqa tools
# using a tool without phpqa
docker run --rm -it ghcr.io/edgedesigncz/phpqa:v1.26.0-php7.2 phploc -v
```

| Image | PHP version | Composer version | Tools versions | 
| ----- | ----------- | ---------------- | ----- |
| `ghcr.io/edgedesigncz/phpqa:v1.26.0-php7.2` | 7.2 | 1.8.0 | Versions that supports symfony2 components from default composer.lock. Not [latest versions](https://github.com/EdgedesignCZ/phpqa/issues/159#issuecomment-452794397). |
| `ghcr.io/edgedesigncz/phpqa:v1.26.0-php8.1` | 8.1 | 2.2.12 | Generally, latest versions available at the moment. If you need different versions, then [build custom docker image](https://github.com/EdgedesignCZ/phpqa/issues/210) |
...
```bash
docker run --rm -it ghcr.io/edgedesigncz/phpqa:v1.26.0-php7.2 sh -c "php --version && composer --version && composer outdated --direct --all && phpqa tools"
docker run --rm -it ghcr.io/edgedesigncz/phpqa:v1.26.0-php8.1 sh -c "php --version && composer --version && composer outdated --direct --all && phpqa tools"
```
...
test:
  stage: test
  image: ghcr.io/edgedesigncz/phpqa:v1.26.0-php7.2
  variables:
...
jobs:
  qa:
    container: ghcr.io/edgedesigncz/phpqa:v1.26.0-php8.1
